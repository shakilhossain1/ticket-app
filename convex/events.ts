...
export const updateEvent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Ensure new total tickets is not less tha sold tickets
...
const checkAvailability = query({
  ...
  handler: async (ctx, { eventId }) => {
    ...
    const activeOffers = await ctx.db
      ...
      .collect()
      .then(
        (enties) => enties.filter((e) => e.offerExpiresAt ?? 0 > now).length,
      );

    const availablesSpots =
      event.totalTickets - (purchasedCount + activeOffers);

    return {
      available: availablesSpots > 0,
      availablesSpots,
    };
  },
});
...
export const joinWaitingList = mutation({
  ...
  handler: async (ctx, { eventId, userId }) => {
    ...
    if (available) {
      ...
      //   schedule a job to expire thi offer after the offer duration
    ...
    return {
      ...
      message: available
        ? `Ticket offered - you have ${DURATIONS.TICKET_OFFER / (60 * 1000)} minuites to purchase`
...
export const purchaseTicket = mutation({
  ...
  handler: async (ctx, { eventId, userId, waitingListId, paymentInfo }) => {
    ...
    //   veriry waiting list entry esixts and is valid
    ...
    if (event.is_cancelled) {
      console.log("Attempted purchase of cancelled event", { eventId });
    ...
    try {
      ...
      console.log("Updateing waiting list status to purchased");
      await ctx.db.patch(waitingListId, {
        status: WAITING_LIST_STATUS.PURCHASED,
      });

      console.log("Proccessing queue for next person");
