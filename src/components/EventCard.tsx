...
function EventCard({eventId}: { eventId: Id<"events"> }) {
    ...
    const renderQueuePosition = () => {
        ...
        if (queuePosition.position === 2) {
            return <div
                className={'flex flex-col lg:flex-row items-center justify-center p-3 bg-amber-50 rounded-lg border border-amber-100'}>
                <div className="flex items-center">
                    <CircleArrowRight className="w-5 h-5 text-amber-400 mr-2"/>
                    <span className={'text-amber-700 font-medium'}>
                       You're next in lineQ (Queue position: {queuePosition.position})
                    </span>
                </div>
                <div className="flex items-center">
                    <LoaderCircle className="w-4 h-4 text-amber-400 mr-1"/>
        ...
        return (
            <div
                ...
                <div className="flex items-center">
                    <CircleArrowRight className="w-5 h-5 text-blue-400 mr-2 animate-spin"/>
            ...
            return (
                <div className={'mt-4'}>
                    <button
                        ...
                        className={"w-full bg-gray-100 text-gray-900 px-6 py-3 rounded-lg font-medium hover:bg-gray-200 transition-colors duration-200 shodow-sm flex items-center justify-center gap-2"}>
    ...
    return <div
        ...
        className={`bg-white rounded-xl  transition-all duration-300 cursor-pointer overflow-hidden relative ${isPastEvent ? "opacity-75 hover:opacity-100" : ""}`}
    >
        {/*  event Image */}
        {imageUrl && (
            ...
            <div className="flex justify-between items-start">
                <div>
                    <div className="flex flex-col itmes-start gap-2">
                        {isEventOwner && (
                            <span
                                className={'inline-flex w-fit items-center  gap-1 bg-blue-600/90 text-white px-2 py-1 rounded-full text-xs font-medium'}>
